...
def evaluate(model, dataset:IterableDataset, num_sessions,item_num, args):
    ...
    if num_sessions>10000:
        sessionIndexes = sorted(random.sample(range(0, num_sessions), 10000))
    else:
    ...
    for si in sessionIndexes:
        ...
        item_idx = [test[0]]
        for _ in range(100):
            item_idx.append(random.choice(rated))
