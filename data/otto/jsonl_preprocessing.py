...
trainOutFolder="jsonl_processed"
testFile="test.jsonl"
testOutFolder="jsonl_processed"
...
def handleData(outFolder:str,sourceFile:str,data_maps=[]):
    ...
    with open(sourceFile) as infile:
        flag=True
        with open(f"{outFolder}/{sourceFile}","+w") as outfile:
            while flag:
...
def main():
    cTypeMap,cAidMap,cEventSizeFilter,cEventSort=DM_TypeMap(type_map={"clicks":0,"carts":1,"orders":2}),StatedDM_PaddedAidMap("aid"),DM_EventSizeFilter(2),DM_EventSort(target="ts")
    ...
    write_stats("jsonl_processed/stats.json", #filename
                {"num_items":num_items},
